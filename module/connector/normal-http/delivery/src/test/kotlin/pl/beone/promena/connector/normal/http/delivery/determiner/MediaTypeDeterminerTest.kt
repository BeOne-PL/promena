...
class MediaTypeDeterminerTest {
    ...
    fun `determine _ given dataDescriptor-mediaType-mimeType and dataDescriptor-mediaType-charset`() {
        MediaTypeDeterminer.determine(
            null, (createHeaders(APPLICATION_PDF.mimeType, UTF_16.name(), null) + additionalHeaders).toHttpHeaders()
    ...
    fun `determine _ given dataDescriptor-mediaType-mimeType, without dataDescriptor-mediaType-charset _ should use UTF-8 charset`() {
        MediaTypeDeterminer.determine(
            null, (createHeaders(APPLICATION_PDF.mimeType, null, null) + additionalHeaders).toHttpHeaders()
    ...
    fun `determine _ Content-Type header with charset`() {
        MediaTypeDeterminer.determine(
            null, (createHeaders(null, null, "text/plain; charset=utf-16") + additionalHeaders).toHttpHeaders()
    ...
    fun `determine _ Content-Type header without charset _ should use UTF-8 charset`() {
        MediaTypeDeterminer.determine(
            null, (createHeaders(null, null, "text/plain") + additionalHeaders).toHttpHeaders()
        ) shouldBe TEXT_PLAIN
    }
    ...
    fun `determine _ no dataDescriptor-mediaType-mimeType or Content-Type header without field name _ shouldThrowIllegalStateException`() {
        shouldThrow<IllegalStateException> {
            MediaTypeDeterminer.determine(
                null, (createHeaders(null, null, null) + additionalHeaders).toHttpHeaders()
            )
        }.message shouldBe "Headers of one of the parts don't contain <dataDescriptor-mediaType-mimeType> or <Content-Type> header"
    }
