...
class MemoryWithBackPressureIncomingExternalCommunicationConverter : IncomingExternalCommunicationConverter {
    ...
    override fun convert(dataDescriptors: List<DataDescriptor>,
        ...
        return if (externalCommunicationId != internalCommunicationId) {
            ...
            dataDescriptors.convertToMemoryDataDescriptors()
        } else {
            dataDescriptors
        }
    }

    private fun List<DataDescriptor>.convertToMemoryDataDescriptors(): List<DataDescriptor> =
            map { DataDescriptor(it.data.createMemoryDataAndDeleteOldDataResource(logger), it.mediaType, it.metadata) }
}
