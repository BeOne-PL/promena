...
@ContextConfiguration(classes = [TransformerController::class, FromSpringMvcParametersMapToCommunicationParametersConverterContext::class])
class TransformerControllerTest {
    ...
    fun `transform _ with empty communication parameters`() {
        val emptyCommunicationParameters = MapCommunicationParameters(emptyMap())

        `when`(fromMapToCommunicationParametersConverter.convert(emptyMap())).thenReturn(emptyCommunicationParameters)
        `when`(transformationUseCase.transform(eq("default"),
                                               eq("request body".toByteArray()),
                                               eq(emptyCommunicationParameters)))
